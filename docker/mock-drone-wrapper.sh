...
ffmpeg -re \
  ...
  -filter_complex "[0:v]drawtext=text='DRONE':fontsize=72:fontcolor=white:borderw=3:bordercolor=black:x=(w-text_w)/2:y=50[vout];[1:a]aformat=channel_layouts=stereo[aout]" \
  -map "[vout]" -map "[aout]" \
